...
function WinnersPerYear() {
	...
	const fetchWinnersPerYear = async () => {
		...
		const { data } = await axios.get(
			"https://tools.texoit.com/backend-java/api/movies?winner=true&year=" +
				yearFilter
		);

		const winnersPerYear = data.map((movie) => {
			return {
				id: movie.id,
				title: movie.title,
				year: movie.year,
	...
	return (
		<>
			<div className="per-year-filter">
				...
				<DefaultButton
					content="Filter!"
					handleClick={fetchWinnersPerYear}
				/>
			</div>
			<Table
				columns={["Id", "Year", "Title"]}
				tableData={winnersPerYear}
			/>
		</>
	);
}
